...
export default function StudentsPage() {
    ...
    // useSWR for fetching students with pagination and search
    const { data: swrData, error, mutate, isLoading } = useSWR(
        mounted ? `/api/students?page=${page}&limit=10&search=${search}` : null,
        fetcher,
    ...
    const handleSubmit = async (e) => {
        ...
        try {
            ...
            if (res.ok) {
                showToast(editingId ? "Data berhasil diperbarui!" : "Siswa baru berhasil ditambahkan!");
                resetForm();
        ...
        <div className="ripple-effect">
                    ...
                    <table className="data-table" style={{ border: 'none', borderCollapse: 'collapse', margin: 0, width: '100%' }}>
                        <thead style={{ backgroundColor: '#F8FAFC', borderBottom: '2px solid #E2E8F0' }}>
                            <tr>
                                ...
                                <th style={{ verticalAlign: 'middle', borderBottom: 'none', padding: '12px 16px', color: '#64748B', fontSize: '11px', textTransform: 'uppercase', letterSpacing: '0.05em' }}>NAMA SISWA</th>
                                ...
                                <th style={{ verticalAlign: 'middle', borderBottom: 'none', padding: '12px 16px', color: '#64748B', fontSize: '11px', textTransform: 'uppercase', letterSpacing: '0.05em' }}>KELAS</th>
                                <th style={{ verticalAlign: 'middle', borderBottom: 'none', padding: '12px 16px', color: '#64748B', fontSize: '11px', textTransform: 'uppercase', letterSpacing: '0.05em' }}>TANGGAL DAFTAR</th>
                        ...
                        <tbody className="zebra-rows">
                            {isLoading ? (
                                ...
                                <tr>
                                    <td colSpan="6" style={{ textAlign: "center", color: "var(--text-light)", padding: "32px 24px" }}>
                                ...
                                <tr key={student._id}>
                                    ...
                                    <td style={{ fontWeight: 600, color: 'var(--primary)', verticalAlign: 'middle', padding: '8px 16px' }}>{student.name}</td>
                                    <td style={{ verticalAlign: 'middle', padding: '8px 16px' }}>{student.contact}</td>
                                    <td style={{ fontWeight: 500, verticalAlign: 'middle', padding: '8px 16px' }}>{student.grade}</td>
                                    <td style={{ color: '#64748B', fontSize: '13px', verticalAlign: 'middle', padding: '8px 16px' }}>
                                    ...
                                    <td style={{ textAlign: "right", whiteSpace: "nowrap", verticalAlign: 'middle', padding: '8px 24px 8px 16px' }}>
                                        <div style={{ display: "flex", gap: "8px", justifyContent: "flex-end" }}>
                                            ...
                                            <button onClick={() => setDeleteId(student._id)} className="btn-action" style={{ color: '#EF4444' }} title="Hapus">
            ...
            >
                <form onSubmit={handleSubmit} style={{ display: "flex", flexDirection: "column", gap: "20px" }}>
                    <div>
                        <label style={{ display: 'block', marginBottom: '8px', fontSize: '14px', fontWeight: 600, color: '#475569' }}>Nama Lengkap Siswa</label>
                        <input
                            type="text"
                    ...
                    <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '20px' }}>
                        ...
                        <div>
                            <label style={{ display: 'block', marginBottom: '8px', fontSize: '14px', fontWeight: 600, color: '#475569' }}>Kelas Saat Ini</label>
                            <input
                                type="text"
                                placeholder="Misal: 5 SD"
